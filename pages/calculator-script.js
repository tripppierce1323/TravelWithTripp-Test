...
function getYouTubeLink(card) {
  return card.youtubeUrl || card.youtube || card.videoUrl || card.reviewUrl || "";
}
  ...
  if (container) {
    container.innerHTML = `
      <div class="results-placeholder">
        ...
        <p>Select your categories and filters, then click <strong>"Find My Cards"</strong> to get personalized recommendations.</p>
      </div>
    `;
  }
}
...
function passesFilter(card, selectedFilters) {
  ...
  const typeFilters = ["flexible", "cashback", "airline", "hotel", "business", "beginner", "premium"];
  const brandFilters = ["delta", "united", "american", "southwest", "hilton", "marriott", "hyatt", "ihg", "aircanada", "britishairways", "aerlingus", "iberia"];
  ...
  if (selectedIssuers.length > 0 && !selectedIssuers.includes(card.issuerTag)) return false;
  if (selectedBrands.length > 0 && !selectedBrands.includes(card.brand)) return false;

  const selectedMainTypes = selectedTypes.filter(f => ["flexible", "cashback", "airline", "hotel"].includes(f));

  if (selectedMainTypes.length > 0 && !selectedMainTypes.includes(card.type)) return false;
  if (selectedTypes.includes("business") && card.business !== true) return false;
  if (selectedTypes.includes("beginner") && card.beginnerFriendly !== true) return false;
  if (selectedTypes.includes("premium") && card.premium !== true) return false;
...
function shouldShowBusinessCards(card, selectedFilters) {
  ...
  if (businessSelected) return true;
...
function hasHotelBrandFilter(selectedFilters) {
  return selectedFilters.some(f => ["hilton", "marriott", "hyatt", "ihg"].includes(f));
}

function hasAirlineBrandFilter(selectedFilters) {
  return selectedFilters.some(f => ["delta", "united", "american", "southwest", "aircanada", "britishairways", "aerlingus", "iberia"].includes(f));
}
...
function getCategoryScore(card, category, selectedFilters, goal) {
  ...
  if (isEverydayCategory && card.type === "flexible") score += 20;
  if (isEverydayCategory && card.type === "cashback") score += 35;

  if (isEverydayCategory && card.type === "hotel") {
    if (!selectedFilters.includes("hotel") && !hasHotelBrandFilter(selectedFilters)) score -= 35;
  }

  if (isEverydayCategory && card.type === "airline") {
    if (!selectedFilters.includes("airline") && !hasAirlineBrandFilter(selectedFilters)) score -= 35;
  }

  if (card.brand === "bilt" && category !== "rent" && !selectedFilters.includes("bilt")) score -= 60;
  ...
  const premiumRelevant = goal === "luxuryTravel" || selectedFilters.includes("premium") || category === "flightsDirect" || category === "hotelsDirect";

  if (card.premium && !premiumRelevant && isEverydayCategory) score -= 30;
...
function getGoalScore(card, goal) {
  let score = 0;
  if (goal === "luxuryTravel" && card.premium) score += 55;
...
function calculateRecommendationScore(card, selectedCategories, selectedFilters, goal) {
  ...
  if (selectedCategories.length === 0) {
    score += 20;
    if (card.type === "flexible") score += 45;
  ...
  if (card.type === "flexible") score += 15;
  if (card.type === "cashback") score += 12;
...
function renderCard(cardResult) {
  ...
  const youtubeLink = getYouTubeLink(card);

  return `
    <div class="mini-card">
      <img src="${card.imageUrl || 'assets/cards/default-card.png'}" />
      <h4>${card.name}</h4>
      ...
      <div class="card-detail-block"><strong>Top Multipliers</strong>${renderList(card.multipliers || card.benefits || [])}</div>
      <div class="card-detail-block"><strong>Current Welcome Bonus</strong><p>${getWelcomeBonus(card)}</p></div>
      <div class="card-detail-block"><strong>My Review</strong><p>${card.myReview || ""}</p></div>

      <div class="card-detail-block youtube-review-block">
        <strong>YouTube Review</strong>
        ${
          youtubeLink
          ? `<a href="${youtubeLink}" target="_blank" class="video-btn youtube-full-btn">▶ Watch My Full Review</a>`
          : `<p>Video review coming soon.</p>`
        }
      </div>

      <div class="card-detail-block"><strong>Good For</strong>${renderList(card.goodFor || [])}</div>
      <div class="card-detail-block"><strong>Not Good For</strong>${renderList(card.notGoodFor || [])}</div>
      <div class="card-detail-block"><strong>Best Pairing</strong><p>${formatBestPairing(card.bestPairing)}</p></div>

      <div class="actions">
        <a href="${card.applyUrl || '#'}" target="_blank" class="apply-btn">Apply Now</a>
        ${youtubeLink ? `<a href="${youtubeLink}" target="_blank" class="video-btn">Watch Review</a>` : ""}
      </div>
    </div>
  `;
}
...
function renderResults(results) {
  ...
  if (!results.length) {
    container.innerHTML = `<div class="results-placeholder"><h2>No cards found</h2><p>No cards matched your filters.</p></div>`;
    return;
  }

  container.innerHTML = `<div class="other-cards">${results.map(renderCard).join("")}</div>`;
}
...
function runCalculator() {
  ...
  const personalCards = window.creditCards || [];
